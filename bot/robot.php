...
class LinkedInBot{
    ...
    public function __construct($username,$password,$maxCycles,$sleepBetweenCycles,$likesOnFeed,$maxNewConnections,$host){
        $this->host = $host;
    ...
    public function autoInvite(){
        while($this->maxNewConnections == 0 || $this->invites <= $this->maxNewConnections){
            ...
            do{
                $elements =  $this->driver->findElements(WebDriverBy::cssSelector('.mn-pymk-list__cards'));
                foreach ($elements as $i=>$element){
                    ...
                    echo '        '.$this->invites.'/'.$this->maxNewConnections.' => Connecting with '.$user->getText().'...'.PHP_EOL;

                    $element->findElement(WebDriverBy::cssSelector('.button-secondary-small'))->click();
                    sleep(rand(2,4));
